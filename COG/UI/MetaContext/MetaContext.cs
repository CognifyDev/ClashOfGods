...
#pragma warning disable CS8603
    ...
    public class MouseOverPopup : MonoBehaviour
    {
        ...
        public void Awake()
        {
            background = GameObjectUtils.CreateObject<SpriteRenderer>("Background", transform, Vector3.zero, LayerMask.NameToLayer("UI"));
            background.sprite = ResourceUtils.SharpWindowBackgroundSprite.GetSprite();
            ...
            valueViewer = GameObjectUtils.CreateObject<SpriteRenderer>("Value", transform, new(0f, 0f, -1f), LayerMask.NameToLayer("UI"));
            ...
            icon = GameObjectUtils.CreateObject<SpriteRenderer>("Icon", transform, new(0f, 0f, -1f), LayerMask.NameToLayer("UI"));
            icon.gameObject.SetActive(false);

            var group = GameObjectUtils.CreateObject<SortingGroup>("Group", transform, Vector3.zero);
            mask = GameObjectUtils.CreateObject<SpriteMask>("Mask", group.transform, Vector3.zero);
        ...
        public void SetContextOld(PassiveUiElement related, IMetaContextOld context)
        {
            ...
            var pos = CameraUtils.ScreenToWorldPoint(Input.mousePosition, LayerMask.NameToLayer("UI"));
            pos.z = -800f;
            transform.position = pos - (Vector3)anchorPoint;

            //範囲外にはみ出た表示の是正
            {
                var lower = CameraUtils.ScreenToWorldPoint(new(10f, 10f), LayerMask.NameToLayer("UI"));
                var upper = CameraUtils.ScreenToWorldPoint(new(Screen.width - 10f, Screen.height - 10f), LayerMask.NameToLayer("UI"));
                float diff;
        ...
        public void UpdatePosition(Vector2? screenPosition = null, bool smoothedCenter = false)
        {
            ...
            var pos = CameraUtils.ScreenToWorldPoint(screenPos, LayerMask.NameToLayer("UI"));
            pos.z = -800f;
            transform.position = pos - (Vector3)anchorPoint;
    ...
    public class GUIScrollView : AbstractGUIContext
    {
        ...
        {
            var view = GameObjectUtils.CreateObject("ScrollView", null, new Vector3(0f, 0f, 0f), LayerMask.NameToLayer("UI"));
            var inner = GameObjectUtils.CreateObject("Inner", view.transform, new Vector3(-0.2f, 0f, -0.1f));
            var innerSize = Size - new Vector2(0.4f, 0f);

            if (WithMask)
            {
                view.AddComponent<SortingGroup>();
                var mask = GameObjectUtils.CreateObject<SpriteMask>("Mask", view.transform, new Vector3(-0.2f, 0, 0));
                mask.sprite = VanillaAsset.FullScreenSprite;
    ...
    public class GameObjectGUIWrapper : AbstractGUIContext
    {
        private Func<(GameObject gameObject, Size size)> generator;

        public GameObjectGUIWrapper(GUIAlignment alignment, Func<(GameObject gameObject, Size size)> generator) : base(alignment)
        {
        ...
        public GameObjectGUIWrapper(GUIAlignment alignment, IMetaParallelPlacableOld widget) : base(alignment)
        {
            this.generator = () =>
            {
                var holder = GameObjectUtils.CreateObject("Holder", null, UnityEngine.Vector3.zero, LayerMask.NameToLayer("UI"));
    ...
    {
        static public GUIManager Instance { get; private set; } = null!;
        ...
        static GUIManager()
        {
            ClassInjector.RegisterTypeInIl2Cpp<GUIManager>();
        }
        ...
        public void Awake()
        {
            ...
            mouseOverPopup = GameObjectUtils.CreateObject<MouseOverPopup>("MouseOverPopup", transform, Vector3.zero);
        }
    ...
    public class GUIContextEngine : GUI
    {
        public static GUIContextEngine Instance { get; private set; } = new();
        ...
        public GUIContext Image(GUIAlignment alignment, Image image, FuzzySize size) => new GUIImage(alignment, image, size) { IsMasked = true };
        ...
        public GUIContext LocalizedText(GUIAlignment alignment, TextAttributes attribute, string translationKey) => new GUIText(alignment, attribute, new TranslateTextComponent(translationKey));

        public GUIContext RawText(GUIAlignment alignment, TextAttributes attribute, string rawText) => new GUIText(alignment, attribute, new RawTextComponent(rawText));

        public GUIContext Text(GUIAlignment alignment, TextAttributes attribute, TextComponent text) => new GUIText(alignment, attribute, text);

        public GUIContext Margin(FuzzySize margin) => new GUIMargin(GUIAlignment.Center, new(margin.Width ?? 0f, margin.Height ?? 0f));
    ...
    public class GUIImage : AbstractGUIContext
    {
        ...
        public GUIImage(GUIAlignment alignment, Image image, FuzzySize size, Color? color = null) : base(alignment)
        {
            Image = image;
            Size = size;
            Color = color;
        ...
        internal override GameObject Instantiate(Size size, out Size actualSize)
        {
            var renderer = GameObjectUtils.CreateObject<SpriteRenderer>("Image", null, Vector3.zero, LayerMask.NameToLayer("UI"));
    ...
    public class GUIButton : GUIText
    {
        ...
        internal override GameObject Instantiate(Size size, out Size actualSize)
        {
            ...
            var button = GameObjectUtils.CreateObject<SpriteRenderer>("Button", null, Vector3.zero, LayerMask.NameToLayer("UI"));
    ...
    public class GUIMargin : AbstractGUIContext
    {
        protected Vector2 margin;

        public GUIMargin(GUIAlignment alignment, Vector2 margin) : base(alignment)
        {
            this.margin = margin;
        }
    ...
    public class GUIText : AbstractGUIContext
    {
        ...
        public GUIText(GUIAlignment alignment, TextAttributes attribute, TextComponent text) : base(alignment)
        {
            Attr = attribute;
            Text = text;
        }
        ...
        internal override GameObject Instantiate(Size size, out Size actualSize)
        {
            ...
            if (AllowGenerateCollider && (OverlayContext != null || OnClickText != null))
            {
                ...
                {
                    button.OnMouseOver.AddListener((Action)(() => GUIManager.Instance.SetHelpContext(button, OverlayContext())));
                    button.OnMouseOut.AddListener((Action)(() => GUIManager.Instance.HideHelpContextIf(button)));
                }
                if (OnClickText != null)
    ...
    public class GUITextField : AbstractGUIContext
    {
        ...
        internal override GameObject Instantiate(Size size, out Size actualSize)
        {
            ...
            var obj = GameObjectUtils.CreateObject("TextField", null, UnityEngine.Vector3.zero);

            var field = GameObjectUtils.CreateObject<TextField>("Text", obj.transform, new UnityEngine.Vector3(0, 0, -0.1f));
            ...
            var background = GameObjectUtils.CreateObject<SpriteRenderer>("Background", obj.transform, UnityEngine.Vector3.zero, LayerMask.NameToLayer("UI"));
            background.sprite = IsSharpField ? ResourceUtils.SharpWindowBackgroundSprite.GetSprite() : VanillaAsset.TextButtonSprite;
            background.drawMode = SpriteDrawMode.Sliced;
            background.tileMode = SpriteTileMode.Continuous;
    ...
    public class GUIFixedView : AbstractGUIContext
    {
        ...
        internal override GameObject Instantiate(Size size, out Size actualSize)
        {
            var view = GameObjectUtils.CreateObject("FixedView", null, new Vector3(0f, 0f, 0f), LayerMask.NameToLayer("UI"));
            var inner = GameObjectUtils.CreateObject("Inner", view.transform, new Vector3(-0.2f, 0f, -0.1f));
            var innerSize = Size - new Vector2(0.4f, 0f);
