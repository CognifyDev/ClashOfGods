...
using static COG.UI.MetaContext.IMetaContextOld;
using COG.Utils;
...
#pragma warning disable CS8601
    ...
    public class MetaScreen : MonoBehaviour, GUIScreen
    {
        ...
        static private MultiImage closeButtonSprite = DividedSpriteLoader.FromResource("COG.Resources.MetaContext.CloseButton.png", 100f, 2, 1);
        static readonly private Image backFrameSprite = new ResourceExpandableSpriteLoader("COG.Resources.MetaContext.Background_Frame.png", 150f, 60, 60);
        static readonly private Image backInnerSprite = new ResourceExpandableSpriteLoader("COG.Resources.MetaContext.Background_Inner.png", 150f, 60, 60);
        ...
        {
            var window = GameObjectUtils.CreateObject("MetaWindow", parent, localPos);
            if (backgroundSetting == BackgroundSetting.Old)
            {
                var renderer = GameObjectUtils.CreateObject<SpriteRenderer>("Background", window.transform, new Vector3(0, 0, 0.1f));
            ...
            {
                var inner = GameObjectUtils.CreateObject<SpriteRenderer>("Inner", window.transform, new Vector3(0, 0, 0.1f));
                ...
                var frame = GameObjectUtils.CreateObject<SpriteRenderer>("Frame", window.transform, new Vector3(0, 0, -0.01f));
            ...
            if (withBlackScreen)
            {
                var renderer = GameObjectUtils.CreateObject<SpriteRenderer>("BlackScreen", window.transform, new Vector3(0, 0, 0.2f));
            ...
            if (withClickGuard)
            {
                var collider = GameObjectUtils.CreateObject<BoxCollider2D>("ClickGuard", window.transform, new Vector3(0, 0, 0.2f));
                collider.isTrigger = true;
                collider.gameObject.layer = LayerMask.NameToLayer("UI");
                collider.size = new Vector2(100f, 100f);
                collider.gameObject.SetUpButton();
            }
            var screen = GameObjectUtils.CreateObject<MetaScreen>("Screen", window.transform, Vector3.zero);
            screen.Border = size;
        ...
        static public MetaScreen GenerateBlankWindow(Vector2 size, Transform parent, Vector3 localPos, Vector2 closeButtonPos, bool withBlackScreen)
        {
            ...
            GUIManager.Instance.RegisterUI(obj, button);
            return screen;
        }

        static public PassiveButton InstantiateCloseButton(Transform parent, Vector3 localPos)
        {
            var collider = GameObjectUtils.CreateObject<CircleCollider2D>("CloseButton", parent, localPos);
            collider.isTrigger = true;
            collider.gameObject.layer = LayerMask.NameToLayer("UI");
        ...
        static public MetaScreen GenerateWindow(Vector2 size, Transform parent, Vector3 localPos, bool withBlackScreen, bool closeOnClickOutside, bool withMask = false, BackgroundSetting background = BackgroundSetting.Old)
        {
            ...
            {
                var collider = GameObjectUtils.CreateObject<BoxCollider2D>("CloseButton", obj.transform, new Vector3(-size.x / 2f - 0.3f, size.y / 2f + 0.2f, -1f));
            ...
            if (closeOnClickOutside)
            {
                ...
                var myCollider = GameObjectUtils.CreateObject<BoxCollider2D>("MyScreenCollider", obj.transform, new Vector3(0f, 0f, 0.1f));
            ...
            if (withMask)
            {
                var group = GameObjectUtils.CreateObject<SortingGroup>("Group", obj.transform, Vector3.zero);
                var mask = GameObjectUtils.CreateObject<SpriteMask>("Mask", group.transform, Vector3.zero);
        ...
        public void SetBackImage(Image image, float brightness, float alpha, Vector2 imagePosition, Vector2 maskScale, float imageScale, bool grayout = false)
        {
            ...
            var group = GameObjectUtils.CreateObject<SortingGroup>("BackImage", this.transform, new(0f, 0f, 0.06f));
            group.sortingOrder = -10;
            var mask = GameObjectUtils.CreateObject<SpriteMask>("Mask", group.transform, Vector3.zero);
            mask.sprite = VanillaAsset.FullScreenSprite;
            mask.transform.localScale = maskScale;

            var renderer = GameObjectUtils.CreateObject<SpriteRenderer>("MaskedImage", group.transform, imagePosition);
            ...
            //if (grayout) renderer.material.shader = Achievement.materialShader; 我草这是啥啊。
        }
    }
    ...
    public class TranslateTextComponent : TextComponent
    {
        ...
        public string GetString() => LanguageConfig.Instance.GetString(TranslationKey);
    ...
    public class MetaContextOld : IMetaContextOld, IMetaParallelPlacableOld
    {
        ...
        public class Image : IMetaContextOld, IMetaParallelPlacableOld
        {
            ...
            public float Generate(GameObject screen, Vector2 center, out float width)
            {
                var renderer = GameObjectUtils.CreateObject<SpriteRenderer>("Image", screen.transform, center);
            ...
            public float Generate(GameObject screen, Vector2 cursor, Vector2 size, out (float min, float max) width)
            {
                ...
                if (Sprite)
                {
                    var renderer = GameObjectUtils.CreateObject<SpriteRenderer>("Image", screen.transform, ReflectAlignment(Alignment, mySize + new Vector2(0.1f, 0.1f), cursor, size));
                    renderer.sprite = Sprite;
        ...
        public class Button : IMetaContextOld, IMetaParallelPlacableOld
        {
            ...
            public float Generate(GameObject screen, Vector2 center, out float width)
            {
                var renderer = GameObjectUtils.CreateObject<SpriteRenderer>("Button", screen.transform, center);
            ...
            public float Generate(GameObject screen, Vector2 cursor, Vector2 size, out (float min, float max) width)
            {
                var renderer = GameObjectUtils.CreateObject<SpriteRenderer>("Button", screen.transform,
        ...
        public class StateButton : IMetaContextOld, IMetaParallelPlacableOld
        {
            ...
            public float Generate(GameObject screen, Vector2 center, out float width)
            {
                var obj = GameObjectUtils.CreateObject("CheckBox", screen.transform, center);
                Generate(obj);
            ...
            public float Generate(GameObject screen, Vector2 cursor, Vector2 size, out (float min, float max) width)
            {
                var obj = GameObjectUtils.CreateObject("CheckBox", screen.transform, ReflectAlignment(Alignment, new Vector2(0.25f, 0.25f), cursor, size));
                Generate(obj);
        ...
        public class ScrollView : IMetaContextOld, IMetaParallelPlacableOld
        {
            ...
            public float Generate(GameObject screen, Vector2 center, out float width)
            {
                var view = GameObjectUtils.CreateObject("ScrollView", screen.transform, new Vector3(center.x, center.y, -0.01f));
                var inner = GameObjectUtils.CreateObject("Inner", view.transform, new Vector3(0, 0, 0));
                var innerSize = Size - new Vector2(0.4f, 0f);

                if (WithMask)
                {
                    view.AddComponent<SortingGroup>();
                    var mask = GameObjectUtils.CreateObject<SpriteMask>("Mask", view.transform, new Vector3(0, 0, 0));
                    mask.sprite = VanillaAsset.FullScreenSprite;
            ...
            public float Generate(GameObject screen, Vector2 cursor, Vector2 size, out (float min, float max) width)
            {
                var view = GameObjectUtils.CreateObject("ScrollView", screen.transform, ReflectAlignment(Alignment, Size, cursor, size));
                var inner = GameObjectUtils.CreateObject("Inner", view.transform, new Vector3(0, 0, 0));
        ...
        public class TextInput : IMetaContextOld, IMetaParallelPlacableOld
        {
            ...
            private void Generate(Transform screen, Vector2 center)
            {
                ...
                var background = GameObjectUtils.CreateObject<SpriteRenderer>("Background", obj.transform, Vector3.zero);
                background.sprite = useSharpFieldFlag ? ResourceUtils.LoadSprite("COG.Resources.MetaContext.StatisticsBackground.png", 100f) : VanillaAsset.TextButtonSprite;
        ...
        public class FramedContext : IMetaContextOld, IMetaParallelPlacableOld
        {
            ...
            public float Generate(GameObject screen, Vector2 center, out float width)
            {
                ...
                if (HighlightColor != null)
                {
                    var backGround = GameObjectUtils.CreateSharpBackground(new Vector2(width + extended.x * 1.8f, height + extended.y * 1.8f), HighlightColor.Value, frame.transform);
