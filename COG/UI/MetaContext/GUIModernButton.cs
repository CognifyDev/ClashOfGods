...
using COG.Utils;
using static COG.UI.MetaContext.GUIButtonGroupInstance;
...
/// </summary>
internal class GUIModernButton : GUIText
{
    static private MultiImage modernButtonSprite = DividedExpandableSpriteLoader.FromResource("COG.Resources.MetaContext.GUIButton.png", 150f, 12, 12, 3, 2);
    static private MultiImage modernCheckSprite = DividedSpriteLoader.FromResource("COG.Resources.MetaContext.GUICheckmark.png", 150f, 2, 1);
    ...
    internal override GameObject Instantiate(Size size, out Size actualSize)
    {
        ...
        var button = GameObjectUtils.CreateObject<SpriteRenderer>("Button", null, UnityEngine.Vector3.zero, LayerMask.NameToLayer("UI"));
        ...
        if (WithCheckMark)
        {
            var checkMarkBack = GameObjectUtils.CreateObject<SpriteRenderer>("Checkmark", button.transform, (actualSize.ToUnityVector() * 0.5f).AsVector3(-1f) + new Vector3(-0.05f, -0.05f, 0f));
            ...
            var checkMarkFront = GameObjectUtils.CreateObject<SpriteRenderer>("Checkmark", checkMarkBack.transform, new(0.02f, 0.01f, -0.1f));
        ...
        var piledText = GUIContextEngine.API.ColorTextComponent(piledModernColor, Text);
        var selectedText = GUIContextEngine.API.ColorTextComponent(selectedModernColor, Text);
        ...
        {
            passiveButton.OnMouseOver.AddListener((Action)(() => GUIManager.Instance.SetHelpContext(passiveButton, OverlayContext())));
            passiveButton.OnMouseOut.AddListener((Action)(() => GUIManager.Instance.HideHelpContextIf(passiveButton)));
        }
