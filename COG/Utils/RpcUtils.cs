...
{
    public static RpcWriter StartRpcImmediately(PlayerControl playerControl, byte callId, PlayerControl[]? targets = null)
    {
        var writers = new List<MessageWriter>();
        targets ??= PlayerUtils.GetAllPlayers().ToArray();
        foreach (var control in targets)
        {
            writers.Add(AmongUsClient.Instance.StartRpcImmediately(playerControl.NetId, callId, SendOption.Reliable, control.GetClientID()));
        }
    ...
    public class RpcWriter
    {
        ...
        public void WriteBytesAndSize(byte[] bytes)
        {
            foreach (var messageWriter in _writers)
            {
                messageWriter.WriteBytesAndSize(bytes);
            }
        }
        ...
        public void WriteNetObject(InnerNetObject obj)
        {
            foreach (var messageWriter in _writers)
            { 
                messageWriter.WriteNetObject(obj);   
            }
        }
